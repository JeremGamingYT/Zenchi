...
class FullDistillationPipeline:
    ...
    def distill_step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        # Tronque les logits à la même taille vocab
        ...
        labels[:, -1] = -100  # Ignore la dernière position
...
class CrossArchitectureDistillation:
    ...
    TEACHER_MODELS = [
        "openai/gpt-oss-20b",           # Préféré - 20B params
        "Qwen/Qwen2-72B-Instruct",      # Alternative - 72B params
        "meta-llama/Llama-3.1-70B-Instruct",  # Alternative - 70B
    ...
    def __init__(
        self,
        student: ATLAS,
        teacher_name: str = "openai/gpt-oss-20b",  # GPT-OSS-20B par défaut
...
╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝       ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝╚═╝╚═╝  ╚═══╝ ╚═════╝ 
    ...
    print("\n" + "="*70)
    print("🎓 DISTILLATION CROSS-ARCHITECTURE - CONFIGURATION PRODUCTION")
    print("="*70)
    print("""
    ╔══════════════════════════════════════════════════════════════╗
    ║  Teacher: openai/gpt-oss-20b (20B params)                    ║
    ║  Student: ATLAS (State-Space Model, 494M params)             ║
    ║  Samples: 500,000 prompts diversifiés                        ║
    ║  Epochs: 10 (minimum pour 70-80% capacités)                  ║
    ║  Batch: 32 avec gradient accumulation                        ║
    ║  Temps estimé: 24-72h sur 2x T4 / 1x A100                    ║
    ╚══════════════════════════════════════════════════════════════╝
    """)
    
    distiller = CrossArchitectureDistillation(
        student=model,
        teacher_name="openai/gpt-oss-20b",  # GPT-OSS-20B comme demandé
        config=config
    )
    
    # CONFIGURATION PRODUCTION - NE PAS RÉDUIRE!
    DISTILL_CONFIG = {
        'num_samples': 500_000,      # 500K samples minimum
        'num_epochs': 10,            # 10 epochs pour bon transfert
        'batch_size': 32,            # Batch 32 avec gradient accumulation
        'save_path': "./atlas_distilled_gpt_oss.pt"
    }
    
    print(f"📊 Configuration de distillation:")
    ...
    if teacher_tokenizer is not None:
        ...
        print("   ✅ Utilisation du tokenizer HuggingFace de GPT-OSS-20B!")
    else:
        print("   ⚠️ Tokenizer de simulation utilisé")
    
    print("\n" + "="*70)
    print("✅ DISTILLATION INTENSIVE TERMINÉE!")
