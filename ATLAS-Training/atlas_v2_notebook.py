# ------------------------------------------------------------------
# ATLAS V2 MODEL - NOTEBOOK COMPATIBLE VERSION
# Auto-generated from atlas_core_v2.py
# Contains: Core Model + Loader + Test Suite
# ------------------------------------------------------------------
...
# 🚀 OPTIMISATION V2: Mamba Kernel Check
try:
...
class SelectiveSSM(nn.Module):
    ...
    def ssm_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 🚀 OPTIMIZATION START: FAST KERNEL PATH
        if MAMBA_KERNEL_AVAILABLE and x.is_cuda:
            # Préparation des inputs pour le kernel (B, D, L)
            u = x.permute(0, 2, 1).contiguous()         # (B, D, L)
            delta = dt.permute(0, 2, 1).contiguous()    # (B, D, L)
            A = -torch.exp(self.A_log)                  # (D, N)
            B_curr = B.permute(0, 2, 1).contiguous()    # (B, N, L)
            C_curr = C.permute(0, 2, 1).contiguous()    # (B, N, L)
            D = self.D.contiguous()                     # (D,)
            
            # Appel du kernel optimisé
            # delta_softplus=True car dt n'est pas encore softplus-ed dans cette branche
            y = selective_scan_fn(
                u, delta, A, B_curr, C_curr, D, 
                z=None, 
                delta_bias=None, 
                delta_softplus=True,
...
class ATLASTrainer:
    ...
    def train(self, num_epochs: int = 1):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                # Log & Safety Checks (Every 10 steps)
                if global_step % 10 == 0:
                    avg_loss = np.mean(epoch_losses['total'][-10:])
                    
                    # 🛡️ SAFETY CHECKPOINT: Sauvegarde du meilleur modèle
                    if avg_loss < best_loss:
                        best_loss = avg_loss
                        self.save_checkpoint("./atlas_best_model.pt")
                        pbar.set_postfix({'loss': f'{avg_loss:.4f}', 'best': f'{best_loss:.4f} 💾'})
                    else:
                        pbar.set_postfix({'loss': f'{avg_loss:.4f}', 'best': f'{best_loss:.4f}'})
                    
                    # 🛡️ CATASTROPHE GUARD: Arrêt d'urgence si explosion
                    # Si NaN ou Loss > 5x Best Loss (après warmup 200 steps)
                    if np.isnan(avg_loss) or (global_step > 200 and avg_loss > 5 * best_loss):
                        print(f"\n\n🚨 CATASTROPHE DÉTECTÉE à l'étape {global_step}!")
                        print(f"   Current Loss: {avg_loss:.4f}")
                        print(f"   Best Loss:    {best_loss:.4f}")
                        print("🛑 ARRÊT D'URGENCE pour éviter le sur-apprentissage destructeur.")
                        print("♻️  Restauration du meilleur modèle sauvegardé...")
                        
                        import os
                        checkpoint_path = "./atlas_best_model.pt"
                        
                        if os.path.exists(checkpoint_path):
                            try:
                                self.load_checkpoint(checkpoint_path)
                                print("✅ Meilleur modèle restauré avec succès.")
                            except Exception as e:
                                print(f"⚠️ ÉCHEC RESTAURATION: {e}")
                                print("   Le modèle actuel (corrompu) sera utilisé pour l'évaluation.")
                        else:
    ...
    def save_checkpoint(self, path: str):
        """Sauvegarde un checkpoint de manière atomique (avec gestion erreur)"""
        import os
        temp_path = path + ".tmp"
        try:
            torch.save({
            ...
            # Atomic rename (évite fichiers corrompus)
            if os.path.exists(path):
                os.remove(path)
            os.rename(temp_path, path)
            print(f"💾 Checkpoint sauvegardé: {path}")
        except Exception as e:
            print(f"⚠️ Erreur sauvegarde checkpoint ({e}) - Disque plein? Training continue...")
    ...
    def _generate_samples(self, n: int) -> List[str]:
        ...
        templates = {
            'conversation': [
                ...
                "Question: {query}\nAnswer:",
                "Human: {query}\nAI:",
                "User: {query}\nAssistant: Let me think about this.", # CoT trigger
            ],
            'code': [
                "Write a Python function that {task}:\n```python\n",
                "Implement the following in Python:\n{task}\n```python\n",
                "# {task}\ndef solution():\n",
                "User: Write a python code to {task}.\nAssistant: Certainly. Here is the step-by-step implementation.\n", # CoT trigger
            ],
            'reasoning': [
            ...
            'explanation': [
                ...
                "Can you explain {concept} detailedly?\nAssistant: Sure, let's explore this concept.\n", # CoT trigger
            ],
            'instruction': [
...
# ═══════════════════════════════════════════════════════════════════════════════
# PARTIE 16B-2: OFFLINE DISTILLATION & PACKING (OPTIMIZATION)
# ═══════════════════════════════════════════════════════════════════════════════

class OfflineDistillationGenerator:
    """
    Générateur de données de distillation OFFLINE.
    
    Optimisation majeure:
    1. Pré-calcule les outputs du Teacher (Logits + Hidden)
    2. Sauvegarde sur disque
    3. Permet d'entraîner le Student sans charger le Teacher (Gain de VRAM & Vitesse)
    """
    
    def __init__(self, teacher_model, tokenizer, dataset, device=DEVICE):
        self.teacher = teacher_model
        self.tokenizer = tokenizer
        self.dataset = dataset
        self.device = device
        
    def generate_and_save(self, save_path: str, batch_size: int = 4, top_k: int = 8):
        """Génère et sauvegarde les données distillées"""
        print(f"🚀 Démarrage de la génération Offline ({len(self.dataset)} samples)...")
        print("   Cette étape est lente MAIS ne se fait qu'une fois!")
        
        # Structure de données pour le stockage efficace
        # Idéalement, on utiliserait memmap ou h5py pour des datasets géants
        # Ici, une liste de dictionnaires compressés suffit pour <1M samples
        offline_data = []
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Generating Teacher Outputs"):
                ...
                # Compression: Garde seulement Top-K logits pour économiser l'espace
                # C'est suffisant pour la distillation (les queues de distribution sont du bruit)
                topk_vals, topk_indices = torch.topk(logits, top_k, dim=-1)
                
                # Transfert CPU pour stockage
                input_ids_cpu = input_ids.cpu().to(torch.int32)
                hidden_cpu = hidden.cpu().to(torch.float16)  # FP16 pour gain place
                topk_vals_cpu = topk_vals.cpu().to(torch.float16)
                topk_indices_cpu = topk_indices.cpu().to(torch.int32)
                
                # Stockage par sample
                for i in range(input_ids.shape[0]):
                    # Filtre le padding pour économiser place
                    length = attention_mask[i].sum().item()
                    
                    offline_data.append({
                        'input_ids': input_ids_cpu[i, :length].clone(),
        ...
        # Sauvegarde
        print(f"💾 Sauvegarde de {len(offline_data)} samples dans {save_path}...")
        torch.save(offline_data, save_path)
        print("✅ Données Offline sauvegardées!")


class PackedDistillationDataset(Dataset):
    """
    Dataset optimisé avec SEQUENCE PACKING.
    
    Concatène les exemples courts pour remplir des séquences de 4096 tokens.
    Élimine 99% du padding waste.
    """
    
    def __init__(self, data_path: str, max_seq_len: int = 4096):
        print(f"📂 Chargement des données offline: {data_path}")
    ...
    def _pack_data(self):
        """Algorithme de packing glouton"""
        print("📦 Packing des séquences...")
        ...
        # Token EOS pour séparer les documents (si dispo)
        # On suppose 2 = EOS pour GPT/Mistral souvent, sinon 0
        eos_id = 2 
        
        for sample in self.raw_data:
            seq_len = len(sample['input_ids'])
            
            # Si ça dépasse, on finalise le paquet actuel
            if current_length + seq_len + 1 > self.max_seq_len:
                if current_length > 0:
                    self._add_packed_sample(current_input_ids, current_hidden, current_vals, current_indices)
            ...
            # Ajoute le sample
            current_input_ids.append(sample['input_ids'])
            current_hidden.append(sample['hidden'])
            current_vals.append(sample['topk_vals'])
            current_indices.append(sample['topk_indices'])
            
            # Ajoute EOS "virtuel" (on ne l'ajoute pas aux hidden/logits car le teacher ne l'avait pas, 
            # mais on paddera le reste)
        ...
        print(f"✅ Packing terminé: {len(self.raw_data)} samples -> {len(self.packed_samples)} packed sequences")
        print(f"   Efficacité théorique: {len(self.raw_data) * 50 / (len(self.packed_samples) * self.max_seq_len):.2%}")

    def _add_packed_sample(self, inputs, hiddens, vals, indices):
        """Concatène et pad un paquet"""
        # Concaténation
        cat_input = torch.cat(inputs, dim=0)
        cat_hidden = torch.cat(hiddens, dim=0)
        cat_vals = torch.cat(vals, dim=0)
        cat_indices = torch.cat(indices, dim=0)
        
        # Padding si nécessaire pour atteindre max_seq_len
        pad_len = self.max_seq_len - len(cat_input)
        
        if pad_len > 0:
            cat_input = F.pad(cat_input, (0, pad_len), value=0) # 0 = PAD
            # Hidden: pad avec zéros
            cat_hidden = F.pad(cat_hidden, (0, 0, 0, pad_len), value=0)
            # Logits: pad
            cat_vals = F.pad(cat_vals, (0, 0, 0, pad_len), value=-100) # -100 ignored
            cat_indices = F.pad(cat_indices, (0, 0, 0, pad_len), value=0)
            
            # Attention mask: 1 pour data, 0 pour pad
        ...
        self.packed_samples.append({
            'input_ids': cat_input.long(),
            'hidden': cat_hidden.float(), # Keep as float (converted to bf16 on GPU)
            'topk_vals': cat_vals.float(),
            'topk_indices': cat_indices.long(),
            'attention_mask': attention_mask
        })
...
class FullDistillationPipeline:
    ...
    def __init__(
        ...
        try:
            # Mode default est plus stable que reduce-overhead qui cause des erreurs de CUDAGraphs
            self.student = torch.compile(self.student, mode="default")
            print("✅ torch.compile(mode='default') activé pour le student!")
        except Exception as e:
            print(f"⚠️ torch.compile() non disponible: {e}")
            print("⚡ Mode eager (sans compilation) pour fallback")
    ...
    def distill_step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        # Check if offline data available
        is_offline = 'topk_vals' in batch
        
        # Mixed precision pour 2x speedup
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            
            # ═══ TEACHER LOGIC (OFFLINE vs ONLINE) ═══
            if is_offline:
                # 1. OFFLINE: Retrouve les logits/hidden du batch
                # On n'a que les Top-K pour gagner de la place
                teacher_topk_vals = batch['topk_vals'].to(DEVICE)      # (B, L, K)
                teacher_topk_indices = batch['topk_indices'].to(DEVICE) # (B, L, K)
                teacher_hidden = batch['hidden'].to(DEVICE)            # (B, L, H)
                
                # NOTE: Pour le KL standard, on a besoin de logits denses
                # On recrée un tenseur dense "approximate"
                # Les valeurs non-topk sont mises à -inf (prob ~ 0)
            ...
            # Ensure dims match (vocab size differences)
            ...
            # 2. Hidden State Alignment (Active seulement si shapes match ou projector)
            # Offline hidden est stocké en FP16, student est BF16/FP32
    ...
    def distill(
        ...
        # Pour Windows, num_workers=0 est souvent plus rapide (évite spawn overhead)
...
class CrossArchitectureDistillation:
    ...
    def load_teacher(self) -> bool:
        ...
        try:
            ...
            # Vérifie la VRAM une seule fois
            total_vram = torch.cuda.get_device_properties(0).total_memory / (1024**3)
            print(f"📊 VRAM Détectée: {total_vram:.1f} GB")
            ...
            for model_name in models_to_try:
                try:
                    ...
                    if total_vram > 30:
                        ...
                        print("   💾 VRAM < 30GB -> Mode ÉCO (4-bit) activé.")
    ...
    def distill(
        ...
        """
        Exécute la distillation avec mode OFFLINE automatique.
        """
        print("╔══════════════════════════════════════════════════════════════╗")
        ...
        offline_path = "atlas_offline_data.pt"
        import os
        
        # 1. Vérifie si données offline existent
        if os.path.exists(offline_path):
            print(f"\n✅ Données Offline trouvées: {offline_path}")
            print("   -> Passage direct à l'entraînement FAST (Teacher non chargé)")
            teacher_model = None
            # Need tokenizer for dataset init? Packed uses pre-computed tokens
            # But we need to init pipeline. Pipeline needs tokenizer? Yes for some utils maybe.
            # We'll load tokenizer separately if needed or assume we have it.
            if self.tokenizer is None:
                # Load real tokenizer 
                print("   🔄 Chargement du Tokenizer seul (pour inférence future)...")
                try: 
                    from transformers import AutoTokenizer
                    # Try teacher_name or first preferred model
                    model_to_use = self.teacher_name if hasattr(self, 'teacher_name') else self.TEACHER_MODELS[0]
            ...
            print("\n⚠️ Pas de données Offline trouvées. Démarrage de la génération...")
            ...
            # 3. Génère dataset de prompts
            print(f"\n📊 Génération du dataset de prompts ({num_samples} samples)...")
            prompt_dataset = DistillationDataset(
                self.tokenizer,
                num_samples=num_samples,
                max_length=512
            )
            
            # 4. Exécute Offline Generation
            generator = OfflineDistillationGenerator(
                self.teacher, 
                self.tokenizer, 
                prompt_dataset
            )
            # Force batch_size 64 pour génération rapide sur H100
            generator.generate_and_save(offline_path, batch_size=64, top_k=8)
            
            # 5. Unload Teacher to free VRAM
            print("♻️ Libération de la VRAM (Teacher)...")
            del self.teacher
            import gc
            gc.collect()
            torch.cuda.empty_cache()
        ...
        # 6. Charge Packed Dataset
        print("\n📦 Chargement du PackedDataset (Optimized)...")
        packed_dataset = PackedDistillationDataset(offline_path, max_seq_len=4096)
        
        # 7. Crée le pipeline de distillation
        # On passe teacher_model=None si offline
        self.pipeline = FullDistillationPipeline(
            student_model=self.student,
            teacher_model=teacher_model, # Can be None
            tokenizer=self.tokenizer,
            config=self.config,
            teacher_hidden_size=self.teacher_hidden_size if hasattr(self, 'teacher_hidden_size') else 4096
        )
        
        # 8. Exécute la distillation
...
╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝       ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝╚═╝╚═╝  ╚═══╝ ╚═════╝ 
    ...
    if FAST_TEST_MODE:
        ...
        print("🔥 MODE PRODUCTION (OPTIMISÉ) - Offline Distillation")
        DISTILL_CONFIG = {
            'num_samples': 200_000,     # 200K samples
            'num_epochs': 5,           # 5 epochs
            'batch_size': 4,            # Batch 4 (Packed ~ 128K tokens context)
    ...
    # VERIFIE SI LE MODELE EXISTE DEJA
    save_path = DISTILL_CONFIG['save_path']
    import os
    
    if os.path.exists(save_path):
        print("\n" + "="*70)
        print(f"⏩ MODÈLE DISTILLÉ DÉJÀ PRÉSENT! ({save_path})")
        print("   -> Chargement direct et passage à l'entraînement...")
        print("="*70)
        ...
        try:
            ...
            try:
                from transformers import AutoTokenizer
                print("   🔄 Chargement du tokenizer Mistral/GPT...")
                tokenizer = AutoTokenizer.from_pretrained("mistralai/Mistral-7B-Instruct-v0.2")
                if tokenizer.pad_token is None:
                    tokenizer.pad_token = tokenizer.eos_token
            except:
                print("   ⚠️ Échec chargement tokenizer, utilisation fallback")
                pass
                
        except Exception as e:
            print(f"❌ Erreur chargement modèle ({e}) - On recommence la distillation?")
            # Si erreur, on pourrait relancer, mais ici on arrête pour sécu
            return model, None
    ...
    # Entraînement complet (suppression de la limite de démo)
    # config.max_steps = 10  <-- C'était la cause de l'arrêt prématuré!
...
# if __name__ == "__main__":  # <--- DISABLED FOR NOTEBOOK IMPORT
#     model, inference = main()


# ======================================================================
# NOTEBOOK LOADER & TEST UTILS
# ======================================================================
